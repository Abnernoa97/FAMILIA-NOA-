...
function card(member:any,photos:any[]){
 ...
 return `<button type="button" class="album-card" data-album-id="${esc(member.id)}"><div class="album-cover">${cover?`<img src="${esc(photoUrl(cover.storage_path))}" alt="Álbum de ${esc(member.name)}">`:'<span class="album-empty">♡</span>'}</div><span class="album-badge">${mine.length}</span><div class="album-info"><b>Álbum de ${esc(member.name)}</b><span>${mine.length?`${mine.length} foto${mine.length===1?'':'s'}`:'Sin fotos todavía'}</span></div></button>`
}
...
async function renderCurrent(){
 ...
 rendering=true
 try{if(activeAlbumId)await renderAlbum(activeAlbumId);else await renderList()}
 finally{
  rendering=false
