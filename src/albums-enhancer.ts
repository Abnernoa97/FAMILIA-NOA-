...
async function loadData(){const [mr,pr]=await Promise.all([supabase.from('family_members').select('id,name,active').eq('active',true).order('created_at'),supabase.from('photos').select('id,uploader_id,storage_path,created_at,mime_type,file_size,width,height').order('created_at',{ascending:false}).limit(200)]);return {members:(mr.data||[]) as any[],photos:(pr.data||[]) as any[],error:mr.error||pr.error}}
...
async function upload(files:FileList|File[],status:HTMLElement){
 ...
 const {members}=await loadData();const me=members.find(m=>m.id===meId);if(!me){status.textContent='Tu perfil ya no está disponible.';status.classList.add('error');return}
