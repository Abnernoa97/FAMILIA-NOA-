...
function openViewer(index:number){
 ...
 ensureViewer();viewerIndex=Math.max(0,Math.min(index,viewerPhotos.length-1));showViewerPhoto(viewerIndex);const v=document.getElementById('familyPhotoViewer')!;v.hidden=false;document.body.style.overflow='hidden'
}
function showViewerPhoto(index:number){
 if(index<0||index>=viewerPhotos.length)return
 viewerIndex=index;const p=viewerPhotos[index],v=document.getElementById('familyPhotoViewer');if(!v)return
 const img=v.querySelector('.family-photo-main') as HTMLImageElement;img.src=photoUrl(p.storage_path);img.alt='Foto familiar';(v.querySelector('.family-photo-meta') as HTMLElement).textContent=`${index+1} / ${viewerPhotos.length}`
 ;(v.querySelector('.family-photo-prev') as HTMLElement).hidden=index===0;(v.querySelector('.family-photo-next') as HTMLElement).hidden=index===viewerPhotos.length-1
}
...
async function loadData(){
 const [mr,pr]=await Promise.all([supabase.from('family_members').select('id,name,active').eq('active',true).order('created_at'),supabase.from('photos').select('id,uploader_id,storage_path,created_at,mime_type,file_size,width,height').order('created_at',{ascending:false}).limit(200)])
...
async function upload(files:FileList|File[],status:HTMLElement){
 ...
 const {members}=await loadData();const me=members.find(m=>m.name===currentName());if(!me){status.textContent='No se pudo identificar tu perfil.';status.classList.add('error');return}
 const {count}=await supabase.from('photos').select('id',{count:'exact',head:true}).eq('uploader_id',me.id);const existing=count||0,remaining=MAX_PHOTOS-existing
 ...
 for(let i=0;i<selected.length;i++){const file=selected[i];try{
  ...
  const path=`${me.id}/${crypto.randomUUID()}.jpg`;const {error:upErr}=await supabase.storage.from(BUCKET).upload(path,blob,{contentType:'image/jpeg',cacheControl:'31536000',upsert:false});if(upErr)throw upErr
  const {error:dbErr}=await supabase.from('photos').insert({uploader_id:me.id,storage_path:path,caption:null,mime_type:'image/jpeg',file_size:blob.size,width,height});if(dbErr){await supabase.storage.from(BUCKET).remove([path]);throw dbErr}
...
async function renderAlbum(id:string){const root=document.querySelector<HTMLElement>('[data-photo-page]');if(!root)return;const {members,photos}=await loadData();const member=members.find(m=>m.id===id);if(!member)return;activeAlbumId=id;const mine=photos.filter(p=>p.uploader_id===id);viewerPhotos=mine
 ...
 root.querySelector('#albumBack')?.addEventListener('click',()=>{activeAlbumId=null;renderList()});root.querySelector('#memberAlbumUpload')?.addEventListener('change',e=>{const input=e.target as HTMLInputElement;if(input.files)upload(input.files,root.querySelector('#memberAlbumStatus')!);input.value=''})
