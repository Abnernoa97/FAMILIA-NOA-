...
async function loadAlbumSummary(members:any[]){
 const limit=Math.max(MAX_PHOTOS,Math.min(200,members.length*MAX_PHOTOS))
 const {data,error}=await supabase.from('photos').select('id,uploader_id,storage_path,created_at').order('created_at',{ascending:false}).limit(limit)
