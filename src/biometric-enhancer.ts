...
async function call(action:string,payload:Record<string,unknown>={}){
  const request=supabase.functions.invoke('family-passkeys-v2',{body:{action,...payload}})
  const timeout=new Promise<never>((_,reject)=>setTimeout(()=>reject(new Error('La verificación tardó demasiado.')) ,8000))
...
function biometricScreen(){
 ...
 b.onclick=()=>biometric(b,e,false)
 app.querySelector<HTMLButtonElement>('#profileLogin')!.onclick=()=>{sessionStorage.setItem(BYPASS_KEY,'1');location.reload()}
 return {b,e}
}

async function biometric(button:HTMLButtonElement,error:HTMLElement,automatic=false){
 button.disabled=true;error.textContent='Verificando…'
 try{
  ...
  const {token,options}=await call('auth-options',payload)
  const response=await startAuthentication({optionsJSON:options})
  const r=await call('auth-verify',{token,response})
  if(!r?.profile?.id)throw new Error('No se pudo identificar el perfil.')
  localStorage.setItem(PASSKEY_KEY,'enabled');localStorage.setItem(PASSKEY_MEMBER_KEY,r.profile.id);localStorage.setItem(PASSKEY_CRED_KEY,response.id)
  session(r.profile)
 }catch(e){
  const name=(e as Error).name
  if(name==='NotAllowedError'||name==='AbortError'){
   error.textContent='Puedes entrar con tu perfil.'
  }else if(automatic&&(e instanceof Error&&e.message==='La verificación tardó demasiado.')){
   // Never leave the family blocked on a slow passkey/network request.
   sessionStorage.setItem(BYPASS_KEY,'1')
   location.reload()
   return
  }else{
   error.textContent=e instanceof Error?e.message:'No se pudo entrar. Puedes usar tu perfil.'
  }
...
async function autoBiometric(){
 ...
 setTimeout(()=>void biometric(controls.b,controls.e,true),300)
}
