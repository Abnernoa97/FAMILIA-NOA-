...
function login(error=''){
  adminSyncChannel?.unsubscribe();adminSyncChannel=null
...
async function render(){
  ...
  renderMembers(); await renderAccess()
...
async function credentialSheet(memberId:string){
  const m=members.find(x=>x.id===memberId)!;const el=document.createElement('div');el.className='modal';el.innerHTML=`<div class="modal-card"><button class="x">×</button><p class="kicker">${esc(m.name)}</p><h3>Definir acceso</h3><label>Número de casa de Trinidad<input id="house" inputmode="numeric"></label><label>Apodo familiar<input id="nick"></label><button class="save" id="save">Guardar acceso</button></div>`;document.body.appendChild(el);el.querySelector('.x')!.addEventListener('click',()=>el.remove());el.querySelector('#save')!.addEventListener('click',async()=>{const house=(el.querySelector<HTMLInputElement>('#house')!).value;const nick=(el.querySelector<HTMLInputElement>('#nick')!).value;if(!house||!nick)return;const [houseHash,nickHash]=await Promise.all([hash(house),hash(nick)]);const {error}=await supabase.from('family_access').upsert({member_id:memberId,house_code_hash:houseHash,nickname_hash:nickHash,updated_at:new Date().toISOString()});if(!error){el.remove();toast('Acceso actualizado')}else toast('No se pudo actualizar')})
}
