...
async function start(){await loadMembers();const saved=sessionStorage.getItem(PROFILE_KEY);if(saved){try{const profile=JSON.parse(saved) as {id:string;name:string};const current=members.find(m=>m.id===profile.id&&m.name===profile.name);if(current){memberId=current.id;memberName=current.name;renderHome();startSettingsRealtime();startFamilyRealtime();return}}catch{}sessionStorage.removeItem(PROFILE_KEY)}login()}
start()
