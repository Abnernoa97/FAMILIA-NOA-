...
function ensureCss(){
  if(!document.querySelector('link[data-family-maplibre]')){
    ...
    link.dataset.familyMaplibre='true'
  ...
  style.textContent=`
  .family-map-shell{position:relative;height:min(58dvh,510px);min-height:390px;margin:16px;border-radius:30px;overflow:hidden;background:#ddd8ce;border:1px solid rgba(218,211,198,.9);box-shadow:0 18px 45px rgba(35,31,25,.12);isolation:isolate}
  .family-map-canvas{position:absolute;inset:0}.family-map-canvas .maplibregl-canvas{outline:none}
  .family-map-shell::after{content:"";position:absolute;inset:0;pointer-events:none;background:linear-gradient(180deg,rgba(12,12,12,.18),transparent 30%,transparent 68%,rgba(12,12,12,.34));z-index:2}
  ...
  .family-map-marker{position:relative;display:flex;flex-direction:column;align-items:center;filter:drop-shadow(0 5px 10px rgba(0,0,0,.28));cursor:pointer}.family-map-marker-dot{width:42px;height:42px;border:3px solid #fff;border-radius:50%;display:grid;place-items:center;background:#4c367f;color:#fff;font-weight:800;font-size:13px}.family-map-marker.mine .family-map-marker-dot{background:#171716}.family-map-marker::after{content:"";width:3px;height:12px;border-radius:3px;background:#fff;margin-top:-1px}.family-map-marker-label{margin-top:3px;padding:4px 7px;border-radius:9px;background:rgba(23,23,22,.86);color:#fff;font-size:9px;font-weight:700;white-space:nowrap}
  .family-map-loading,.family-map-error{position:absolute;inset:0;z-index:6;display:grid;place-items:center;padding:30px;text-align:center;background:linear-gradient(145deg,#d9d3c7,#f2eee6);color:#5f5a52;font-size:13px}.family-map-error b{display:block;color:#171716;font:500 22px var(--display-font,Georgia,serif);margin-bottom:7px}.family-map-loading[hidden]{display:none}
  .family-map-popup .maplibregl-popup-content{border-radius:16px;padding:12px 14px;box-shadow:0 12px 34px #0003;font-family:system-ui,sans-serif}.family-map-popup b{display:block;font-size:13px}.family-map-popup small{display:block;margin-top:3px;color:#77736b}.family-map-popup .maplibregl-popup-close-button{font-size:18px;padding:2px 7px}
  .family-map-shell .maplibregl-ctrl-bottom-left,.family-map-shell .maplibregl-ctrl-bottom-right{z-index:3}.family-map-shell .maplibregl-ctrl-attrib{font-size:9px;background:rgba(255,255,255,.75)}.family-map-shell .maplibregl-ctrl-group{border-radius:14px;overflow:hidden;box-shadow:0 5px 18px #0002}
...
function loadMapLibre(){
  if(!maplibrePromise){
    ensureCss()
    maplibrePromise=import(/* @vite-ignore */ MAPLIBRE_JS)
  }
...
function panel(){
  ...
  if(!page)return null
  let shell=page.querySelector<HTMLElement>('[data-family-map]')
  if(shell)return shell
  const locationBox=page.querySelector<HTMLElement>('.locationbox')
  if(!locationBox)return null
  shell=document.createElement('section')
  shell.className='family-map-shell'
  shell.dataset.familyMap='true'
  ...
  if(gated)locationBox.insertAdjacentElement('afterend',shell)
  else locationBox.insertAdjacentElement('beforebegin',shell)
...
async function fetchLocations():Promise<MapLocation[]>{
  ...
  return ((locationsResult.data||[]) as LocationRow[])
    .filter(row=>names.has(row.member_id)&&Number.isFinite(row.latitude)&&Number.isFinite(row.longitude))
    .map(row=>({...row,name:names.get(row.member_id)||'Familia',mine:row.member_id===me}))
}
...
function add3dBuildings(instance:any){
  try{
    ...
    const firstLabel=layers.find((layer:any)=>layer.type==='symbol'&&layer.layout?.['text-field'])?.id
    instance.addLayer({
      id:'familia-buildings-3d',
      source:'familia-openfreemap',
      'source-layer':'building',
      type:'fill-extrusion',
      minzoom:13,
      filter:['!=',['get','hide_3d'],true],
      paint:{
        'fill-extrusion-color':['interpolate',['linear'],['coalesce',['get','render_height'],0],0,'#d8d2c7',80,'#bdb5a8',220,'#968d80'],
...
function fitAll(){
  ...
  if(currentLocations.length===1){
    ...
    map.flyTo({center:[item.longitude,item.latitude],zoom:15.5,pitch:58,bearing:-18,duration:1100,essential:true})
    return
  }
  const bounds=currentLocations.reduce((value,item)=>value.extend([item.longitude,item.latitude]),new maplibreBounds(currentLocations[0]),null as any)
  if(bounds)map.fitBounds(bounds,{padding:{top:95,right:55,bottom:120,left:55},maxZoom:13,duration:1100})
}

function maplibreBounds(first:MapLocation){
  const MapLibre=(window as any).__familiaMapLibre
  return MapLibre?new MapLibre.LngLatBounds([first.longitude,first.latitude],[first.longitude,first.latitude]):null
}

function focus(item:MapLocation){
  if(!map)return
  map.flyTo({center:[item.longitude,item.latitude],zoom:15.8,pitch:62,bearing:-20,duration:1050,essential:true})
}

function renderPeople(){
  const root=document.querySelector<HTMLElement>('[data-map-people]')
  if(!root)return
  ...
  root.querySelectorAll<HTMLButtonElement>('[data-map-member]').forEach(button=>button.addEventListener('click',()=>{
    const item=currentLocations.find(location=>location.member_id===button.dataset.mapMember)
    if(item)focus(item)
  }))
}
...
function renderMarkers(MapLibre:any){
  ...
  currentLocations.forEach(item=>{
    ...
    element.type='button'
    element.className=`family-map-marker${item.mine?' mine':''}`
    element.setAttribute('aria-label',`Ver ubicación de ${item.name}`)
    ...
    element.addEventListener('click',event=>{
      ...
      focus(item)
      new MapLibre.Popup({offset:34,className:'family-map-popup',closeButton:true})
        .setLngLat([item.longitude,item.latitude])
        .setHTML(`<b>${esc(item.name)}${item.mine?' · tú':''}</b><small>${esc(age(item.updated_at))}${Number.isFinite(item.accuracy)?` · ±${Math.round(item.accuracy as number)} m`:''}</small>`)
        .addTo(map)
    })
    const marker=new MapLibre.Marker({element,anchor:'bottom'}).setLngLat([item.longitude,item.latitude]).addTo(map)
    markers.push(marker)
  })
  renderPeople()
}
...
async function refreshMap(){
  ...
  const loading=shell.querySelector<HTMLElement>('[data-map-loading]')
  try{
    const [MapLibre,locations]=await Promise.all([loadMapLibre(),fetchLocations()])
    if(run!==generation||!document.contains(shell))return
    ;(window as any).__familiaMapLibre=MapLibre
    currentLocations=locations
    if(!locations.length){
      if(loading){loading.hidden=false;loading.className='family-map-error';loading.innerHTML='<div><b>Aún no hay ubicaciones</b><span>Cuando alguien comparta su ubicación, aparecerá aquí.</span></div>'}
      return
    }
    if(!map){
      const mine=locations.find(item=>item.mine)||locations[0]
      map=new MapLibre.Map({
        container:shell.querySelector<HTMLElement>('[data-family-map-canvas]')!,
        style:MAP_STYLE,
        center:[mine.longitude,mine.latitude],
        zoom:locations.length===1?15.2:10.5,
        pitch:55,
        bearing:-18,
        maxPitch:75,
      ...
      map.addControl(new MapLibre.NavigationControl({showZoom:false,showCompass:true,visualizePitch:true}),'top-right')
      map.addControl(new MapLibre.AttributionControl({compact:true}),'bottom-right')
      map.on('load',()=>{
        add3dBuildings(map)
        renderMarkers(MapLibre)
        fitAll()
        if(loading)loading.hidden=true
      })
      map.on('error',(event:any)=>console.warn('Family map resource error',event?.error||event))
    }else{
      renderMarkers(MapLibre)
      fitAll()
      if(loading)loading.hidden=true
      window.setTimeout(()=>{try{map.resize()}catch{}},40)
    }
  }catch(error){
    console.error('Family 3D map failed to load',error)
    if(loading){loading.hidden=false;loading.className='family-map-error';loading.innerHTML='<div><b>Mapa no disponible</b><span>La lista de ubicaciones sigue funcionando normalmente.</span></div>'}
  }
}

function scheduleRefresh(){
  if(refreshTimer!==null)window.clearTimeout(refreshTimer)
  refreshTimer=window.setTimeout(()=>{refreshTimer=null;void refreshMap()},180)
}

function attachListObserver(){
  const list=document.querySelector('#familylocations')
...
function scan(){
  ...
  if(!page){
    if(map||document.querySelector('[data-family-map]'))destroyMap()
    return
  }
  panel()
  attachListObserver()
