...
function add3dBuildings(instance:any){
  try{
    ...
    instance.addLayer({id:'familia-buildings-3d',source:'familia-openfreemap','source-layer':'building',type:'fill-extrusion',minzoom:13,filter:['!=',['get','hide_3d'],true],paint:{'fill-extrusion-color':['interpolate',['linear'],['coalesce',['get','render_height'],0],0,'#d8d2c7',80,'#bdb5a8',220,'#968d80'],'fill-extrusion-height':['coalesce',['get','render_height'],['get','height'],4],'fill-extrusion-base':['coalesce',['get','render_min_height'],0],'fill-extrusion-opacity':0.78}},firstLabel)
  }catch(error){console.warn('3D buildings unavailable',error)}
}

function fitAll(){
  if(!map||!currentLocations.length)return
  if(currentLocations.length===1){const item=currentLocations[0];map.flyTo({center:[item.longitude,item.latitude],zoom:15.5,pitch:58,bearing:-18,duration:1000,essential:true});return}
  const MapLibre=(window as any).__familiaMapLibre;if(!MapLibre)return
  const bounds=new MapLibre.LngLatBounds();currentLocations.forEach(item=>bounds.extend([item.longitude,item.latitude]));map.fitBounds(bounds,{padding:{top:95,right:55,bottom:120,left:55},maxZoom:13,duration:1000})
}
function focus(item:MapLocation){if(map)map.flyTo({center:[item.longitude,item.latitude],zoom:15.8,pitch:62,bearing:-20,duration:950,essential:true})}
...
async function refreshMap(){
  ...
  try{
    ...
    if(!map){
      ...
      map=new MapLibre.Map({container:shell.querySelector<HTMLElement>('[data-family-map-canvas]')!,style:MAP_STYLE,center:[mine.longitude,mine.latitude],zoom:locations.length===1?15.2:10.5,pitch:55,bearing:-18,maxPitch:75,attributionControl:false,canvasContextAttributes:{antialias:true}})
