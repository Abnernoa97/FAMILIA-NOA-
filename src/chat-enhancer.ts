...
function badgeStyle() {
  ...
  s.textContent = `
    ...
    .chat-tools{display:flex;gap:4px;align-items:center;margin-top:5px;opacity:.78}
...
function showDialog(title: string, initial: string, onConfirm: (value: string) => Promise<void>) {
  ...
  overlay.querySelector('#chatDialogConfirm')!.addEventListener('click',async()=>{const value=input.value.trim();if(!value)return;const b=overlay.querySelector<HTMLButtonElement>('#chatDialogConfirm')!;b.disabled=true;await onConfirm(value);overlay.remove()})
}
...
async function toggleReaction(messageId: string, reaction: string) {
  const id = currentMemberId(); if (!id) return
...
async function renderReactions(messageId: string) {
  ...
  if (!row) { row=document.createElement('div');row.className='chat-reaction-row';bubble.appendChild(row) }
  const counts = new Map<string,number>();for(const r of data||[])counts.set(r.reaction,(counts.get(r.reaction)||0)+1)
  row.innerHTML = Array.from(counts.entries()).map(([r,n])=>`<button class="chat-reaction" data-react="${esc(r)}">${r} ${n}</button>`).join('')
}
...
async function enhanceBubble(bubble: HTMLElement) {
  ...
  const reactions=['❤️','😂','👍','😮','😢','🙏']
  tools.innerHTML = reactions.map(r=>`<button type="button" data-react="${r}" aria-label="Reaccionar ${r}">${r}</button>`).join('')
  ...
  tools.addEventListener('click',async e=>{
    ...
    if(reaction){await toggleReaction(id,reaction);return}
    if(target.hasAttribute('data-edit')){
      const body=bubble.querySelector('p')?.textContent||''
      showDialog('Editar mensaje',body,async value=>{const {data,error}=await supabase.rpc('edit_chat_message',{p_member_id:currentMemberId(),p_message_id:id,p_body:value});if(!error&&data){const p=bubble.querySelector('p');if(p)p.textContent=value}})
    }
    if(target.hasAttribute('data-delete')){
      if(!confirm('¿Eliminar este mensaje?'))return
      const {data,error}=await supabase.rpc('delete_chat_message',{p_member_id:currentMemberId(),p_message_id:id})
      if(!error&&data){const p=bubble.querySelector('p');if(p)p.textContent='Mensaje eliminado';tools.remove()}
    }
  })
...
function installComposer() {
  const composer=document.querySelector<HTMLElement>('#composer'); if(!composer||composer.dataset.chatFeatures==='1')return
  composer.dataset.chatFeatures='1'
  const input=composer.querySelector<HTMLInputElement>('#message'); if(!input)return
  const file=document.createElement('input');file.type='file';file.accept='image/*';file.hidden=true;file.id='chatAttachmentInput'
  const attach=document.createElement('button');attach.type='button';attach.className='chat-attach-button';attach.textContent='＋';attach.setAttribute('aria-label','Adjuntar foto')
  composer.insertBefore(attach,input);composer.appendChild(file)
  attach.addEventListener('click',()=>file.click())
  file.addEventListener('change',async()=>{
    const selected=file.files?.[0];if(!selected)return
    if(selected.size>10*1024*1024){alert('La foto debe pesar menos de 10 MB.');file.value='';return}
    const id=currentMemberId();if(!id)return
    ...
    if(uploadError){alert('No se pudo subir la foto.');attach.disabled=false;file.value='';return}
    ...
    if(insertError){await supabase.storage.from('family-photos').remove([path]);alert('No se pudo enviar la foto.')}
    attach.disabled=false;file.value=''
  })
  input.addEventListener('input',()=>{
    ...
    typingStopTimer=window.setTimeout(()=>{if(chatChannel)void chatChannel.send({type:'broadcast',event:'typing',payload:{memberId:currentMemberId(),name:identity()?.name||'Familia',typing:false}})},900)
  })
}

async function loadAttachments() {
  const list=document.querySelector<HTMLElement>('#messages'); if(!list)return
  const ids=Array.from(list.querySelectorAll<HTMLElement>('.bubble[data-message-id]')).map(x=>x.dataset.messageId).filter(Boolean) as string[]
  ...
  for(const m of data||[]){
    const bubble=list.querySelector<HTMLElement>(`[data-message-id="${m.id}"]`);if(!bubble||!m.attachment_path||m.deleted_at)continue
    if(bubble.querySelector('.chat-attachment'))continue
    const {data:signed}=await supabase.storage.from('family-photos').createSignedUrl(m.attachment_path,3600)
    if(!signed?.signedUrl)continue
    const link=document.createElement('a');link.className='chat-attachment';link.href=signed.signedUrl;link.target='_blank';link.rel='noreferrer'
    if((m.attachment_type||'').startsWith('image/'))link.innerHTML=`<img src="${esc(signed.signedUrl)}" alt="${esc(m.attachment_name||'Foto')}" loading="lazy"><span>${esc(m.attachment_name||'Foto')}</span>`
    else link.innerHTML=`<span>📎 ${esc(m.attachment_name||'Archivo')}</span>`
...
function initChat() {
  ...
  chatInitialized=true;badgeStyle()
  const list=document.querySelector<HTMLElement>('#messages'); if(!list)return
  ...
  const typing=document.createElement('div');typing.className='chat-typing';typing.id='chatTyping';list.before(typing)
  const indicator=document.createElement('button');indicator.type='button';indicator.className='chat-new-indicator';indicator.hidden=true;indicator.textContent='Nuevos mensajes ↓';list.before(indicator)
  installComposer()
  const enhanceAll=()=>list.querySelectorAll<HTMLElement>('.bubble').forEach(b=>void enhanceBubble(b))
  enhanceAll();void loadAttachments();void markRead();void renderReadReceipts()

  let lastAtBottom=true
  list.addEventListener('scroll',()=>{lastAtBottom=list.scrollHeight-list.scrollTop-list.clientHeight<100;if(lastAtBottom){indicator.hidden=true;void markRead()}},{passive:true})
  indicator.addEventListener('click',()=>{list.scrollTo({top:list.scrollHeight,behavior:'smooth'});indicator.hidden=true;void markRead()})

  chatChannel=supabase.channel('familia-noa-chat-features')
    .on('postgres_changes',{event:'INSERT',schema:'public',table:'messages'},payload=>{
      const row=payload.new as any
      if(row?.sender_id!==currentMemberId()&&!lastAtBottom){indicator.hidden=false;indicator.textContent='Nuevos mensajes ↓'}
      scheduleUnreadRefresh();setTimeout(()=>{enhanceAll();void loadAttachments();void renderReadReceipts()},250)
    })
    .on('postgres_changes',{event:'UPDATE',schema:'public',table:'messages'},()=>setTimeout(()=>{enhanceAll();void loadAttachments();void renderReadReceipts()},150))
    .on('postgres_changes',{event:'*',schema:'public',table:'chat_reactions'},payload=>{const id=(payload.new as any)?.message_id||(payload.old as any)?.message_id;if(id)void renderReactions(id)})
    .on('broadcast',{event:'typing'},payload=>{
      const p=payload.payload as any;if(!p||p.memberId===currentMemberId())return
      const el=document.querySelector<HTMLElement>('#chatTyping');if(!el)return
  ...
  presenceChannel=supabase.channel('familia-noa-chat-presence',{config:{presence:{key:currentMemberId()}}})
    .on('presence',{event:'sync'},()=>{
      ...
      const el=document.querySelector<HTMLElement>('#chatPresence');if(!el)return
      el.textContent=people.length===0?'Solo tú':people.length===1?`${people[0]?.name||'1 persona'} en línea`:`${people.length} familiares en línea`
    })
    .subscribe(async status=>{if(status==='SUBSCRIBED')await presenceChannel?.track({memberId:currentMemberId(),name:identity()?.name||'Familia',onlineAt:new Date().toISOString()})})

  const observer=new MutationObserver(()=>{enhanceAll();void loadAttachments()})
...
function scan(){
  ...
  else { chatInitialized=false;chatChannel?.unsubscribe();chatChannel=null;void presenceChannel?.untrack();presenceChannel?.unsubscribe();presenceChannel=null }
