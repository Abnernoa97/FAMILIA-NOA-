...
const compareMessages=(a:ChatMessage,b:ChatMessage)=>new Date(a.created_at).getTime()-new Date(b.created_at).getTime()||a.id.localeCompare(b.id)
...
function storageUrl(path:string){return mediaUrl(path)}

function audioPlayerHtml(src:string,label:string,path=''){
  const mediaPath=path?` data-media-path="${esc(path)}"`:''
  return `<div class="chat-attachment chat-audio"${mediaPath}><button type="button" class="chat-audio-toggle" aria-label="Reproducir ${esc(label)}">▶</button><div class="chat-audio-track" data-audio-seek><span class="chat-audio-progress"></span></div><span class="chat-audio-time">0:00</span><audio class="chat-audio-player" src="${esc(src)}" preload="metadata" aria-label="${esc(label)}"></audio></div>`
}

function quotedHtml(quoted:ChatMessage|null){
  if(!quoted)return''
  if(quoted.deleted_at)return `<button type="button" class="chat-quoted" data-jump="${esc(quoted.id)}"><b>${esc(quoted.sender?.name||'Familia')}</b><span>Mensaje eliminado</span></button>`
  ...
  if(isVideoMessage(quoted))return `<button type="button" class="chat-quoted" data-jump="${esc(quoted.id)}"><b>${esc(quoted.sender?.name||'Familia')}</b><span>Video</span></button>`
  if(isAudioMessage(quoted))return `<button type="button" class="chat-quoted" data-jump="${esc(quoted.id)}"><b>${esc(quoted.sender?.name||'Familia')}</b><span>${isVoiceMessage(quoted)?'Mensaje de voz':'Audio'}</span></button>`
...
function bubbleHtml(message:ChatMessage,quoted:ChatMessage|null,mine:boolean,priorityMedia=false){
  ...
  return `<article class="chat-bubble${mine?' mine':''}${deleted?' deleted':''}" data-message-id="${esc(message.id)}">${quotedHtml(quoted)}<b class="chat-sender">${esc(message.sender?.name||'Familia')}</b>${body?`<p>${esc(body)}</p>`:''}${attachmentHtml(message,priorityMedia)}<small class="chat-meta">${time(message.created_at)}${message.edited_at?' · editado':''}</small></article>`
}

export function closeChat(){activeCleanup?.();activeCleanup=null}
...
export async function openChat(options:OpenChatOptions){
  ...
  const scrollLatest=()=>{if(list.isConnected)list.scrollTop=Math.max(0,list.scrollHeight-list.clientHeight)}
  ...
  const onAudioClick=(event:Event)=>{
    ...
    if(target.closest('.chat-audio-toggle')){
      ...
      void(async()=>{
        if(audio.paused){
          const path=shell.dataset.mediaPath
          if(path){
            try{
              const fresh=await signMedia(path)
              if(fresh&&audio.src!==fresh){audio.src=fresh;audio.load()}
            }catch(error){console.error('Chat audio authorization failed',error);return}
          }
          if(audio.ended)audio.currentTime=0
          await audio.play().catch(error=>console.error('Chat audio play failed',error))
        }else audio.pause()
      })()
      return
    }
  ...
  const onAudioState=(event:Event)=>{
    ...
    if(audio instanceof HTMLAudioElement&&audio.classList.contains('chat-audio-player'))syncAudioUi(audio)
  }
  ...
  const replaceMessageElement=(id:string)=>{
    const message=byId.get(id),current=elementFor(id)
  ...
  const appendMessage=async(message:ChatMessage,forceBottom=false)=>{
    ...
    if(all.length>MAX_LOADED_MESSAGES){
      ...
      removed.forEach(item=>{elementFor(item.id)?.remove();if(!all.some(loaded=>loaded.reply_to_id===item.id))byId.delete(item.id)})
    }
  ...
  const renderInitial=async()=>{
    try{
      ...
      scrollLatest()
      requestAnimationFrame(()=>{scrollLatest();list.classList.add('is-ready')})
      void document.fonts?.ready.then(()=>{if(!closed&&stickToLatest)scheduleLatest()})
    }catch(error){
  ...
  const onScroll=()=>{stickToLatest=isAtBottom();if(list.scrollTop<=60)void loadOlder()}
  ...
  const renderPendingText=(job:PendingText)=>{
    ...
    stickToLatest=true;scheduleLatest()
  }
  ...
  const setPendingTextState=(id:string,state:'sending'|'error')=>{
    ...
    if(meta)meta.innerHTML=state==='sending'?'<span class="chat-spinner"></span> Enviando…':'<button type="button" class="chat-retry" data-retry-text>Reintentar</button> · No enviado'
  }
  ...
  const finishPendingText=async(job:PendingText,message:ChatMessage)=>{
    pendingTextElement(job.id)?.remove();outbox.done(job.id)
  ...
  const sendTextJob=async(job:PendingText)=>{
    ...
    try{
      ...
      try{const existing=await existingMessage(job.id);if(existing){await finishPendingText(job,existing);return}}catch(lookupError){console.error('Chat text reconciliation failed',lookupError)}
      console.error('Chat text send failed',error);setPendingTextState(job.id,'error');throw error
    }
  }
  ...
  const onSubmit=(event:SubmitEvent)=>{
    ...
    input.value='';features?.clearReply();renderPendingText(job);void outbox.add(job);input.focus()
  }
  ...
  const renderPendingMedia=(job:PendingMedia)=>{
    ...
    const label=job.mediaKind==='video'?'Preparando video…':job.mediaKind==='audio'?'Enviando audio…':'Enviando foto…'
  ...
  const setPendingMediaState=(id:string,state:'sending'|'error')=>{
    ...
    if(meta)meta.innerHTML=state==='sending'?'<span class="chat-spinner"></span> Conectando…':'<button type="button" class="chat-retry" data-retry-media>Reintentar</button> · No enviado'
  }

  const setPendingMediaProgress=(id:string,uploaded:number,total:number)=>{
    const meta=pendingElement(id)?.querySelector<HTMLElement>('[data-pending-state]')
    if(!meta||!total)return
    const percent=Math.max(0,Math.min(100,Math.round(uploaded/total*100)))
    meta.innerHTML=percent>=100?'<span class="chat-spinner"></span> Guardando…':`<span class="chat-spinner"></span> Subiendo ${percent}%`
  }
  ...
  const sendMediaJob=async(job:PendingMedia)=>{
    ...
    try{
      ...
      if(insertError||!data){
        ...
        try{existing=await existingMessage(job.id)}catch(lookupError){console.error('Chat media reconciliation failed',lookupError);throw insertError||new Error('Message insert failed')}
        if(existing){await finishPendingMedia(job,existing);return}
        try{await removeMedia(path)}catch(cleanupError){console.error('Uncommitted Chat media cleanup failed',cleanupError)}
        throw insertError||new Error('Message insert failed')
      }
      await finishPendingMedia(job,nameRow(data as RawChatRow))
    }catch(error:any){
      console.error('Chat media send failed',error);setPendingMediaState(job.id,'error')
      ...
      else if(message.includes('CHAT_VIDEO_SIZE_LIMIT')||message.includes('VIDEO_SOURCE_TOO_LARGE'))notify('Video demasiado grande',`El video debe pesar ${mediaLimitMb(CHAT_VIDEO_MAX_BYTES)} MB o menos.`)
      else if(message.includes('CHAT_AUDIO_SIZE_LIMIT'))notify('Audio demasiado grande',`El audio debe pesar ${mediaLimitMb(CHAT_AUDIO_MAX_BYTES)} MB o menos.`)
      throw error
    }
  }
  ...
  const configurePicker=(kind:MediaKind)=>{
    fileInput.value='';fileInput.dataset.mediaKind=kind
    fileInput.accept=kind==='image'?'image/jpeg,image/png,image/webp,image/heic':kind==='video'?'video/mp4,video/webm,video/quicktime,video/x-m4v':'audio/mpeg,audio/mp4,audio/x-m4a,audio/aac,audio/wav,audio/x-wav,audio/webm,audio/ogg'
  }
  const closeAttachMenu=()=>{attachMenu.hidden=true;attach.setAttribute('aria-expanded','false')}
  const onAttachClick=()=>{if(voiceSession)return;const open=attachMenu.hidden;attachMenu.hidden=!open;attach.setAttribute('aria-expanded',open?'true':'false')}
  const onAttachMenuClick=(event:Event)=>{
    const kind=(event.target as HTMLElement).closest<HTMLButtonElement>('[data-attach-kind]')?.dataset.attachKind as MediaKind|undefined
    if(!kind)return
    configurePicker(kind);closeAttachMenu();fileInput.click()
  }
  const onDocumentPointer=(event:PointerEvent)=>{const target=event.target as Node;if(!attachMenu.hidden&&!attachMenu.contains(target)&&!attach.contains(target))closeAttachMenu()}

  const onAttachment=()=>{
    const files=Array.from(fileInput.files||[]),requestedKind=fileInput.dataset.mediaKind as MediaKind|undefined
    fileInput.value='';delete fileInput.dataset.mediaKind
    for(const file of files){
      ...
      if(isImage&&requestedKind!=='video'&&requestedKind!=='audio'){
        ...
        if(file.size>CHAT_IMAGE_MAX_BYTES){notify('Foto demasiado grande',`La foto debe pesar ${mediaLimitMb(CHAT_IMAGE_MAX_BYTES)} MB o menos.`);continue}
      }else if(isVideo&&requestedKind!=='audio'){
        mediaKind='video'
        if(!isSupportedVideo(file)){notify('Video no compatible','Usa MP4, MOV, M4V o WebM.');continue}
        if(file.size>CHAT_VIDEO_MAX_BYTES){notify('Video demasiado grande',`El video debe pesar ${mediaLimitMb(CHAT_VIDEO_MAX_BYTES)} MB o menos.`);continue}
      }else if(isAudio){
        mediaKind='audio'
        if(!isSupportedAudio(file)){notify('Audio no compatible','Usa MP3, M4A, AAC, WAV, WebM u OGG.');continue}
        if(file.size>CHAT_AUDIO_MAX_BYTES){notify('Audio demasiado grande',`El audio debe pesar ${mediaLimitMb(CHAT_AUDIO_MAX_BYTES)} MB o menos.`);continue}
      }else{notify('Archivo no compatible','Selecciona una foto, un video o un audio.');continue}
      const job:PendingMedia={kind:'media',mediaKind,id:crypto.randomUUID(),file,objectUrl:URL.createObjectURL(file),replyToId:features?.getReplyToId()||null,busy:false}
      renderPendingMedia(job);void outbox.add(job)
    }
  }

  const stopVoiceClock=()=>{if(voiceTimer!==null)window.clearInterval(voiceTimer);voiceTimer=null}
  const setVoiceUi=(recording:boolean)=>{
    composer.classList.toggle('is-recording',recording);voiceRecording.hidden=!recording
    if(!recording)voiceTime.textContent='0:00'
    if(recording){closeAttachMenu();input.blur();stickToLatest=true;scheduleLatest()}
  }

  const startVoiceMessage=async()=>{
    if(voiceBusy||voiceSession)return
    if(!canRecordVoice()){notify('Micrófono no disponible','Este dispositivo o navegador no permite grabar mensajes de voz.');return}
    voiceBusy=true;voiceButton.disabled=true
    try{
      const session=await beginVoiceRecording()
      if(closed){await session.cancel();return}
      voiceSession=session;setVoiceUi(true);voiceTime.textContent='0:00'
      voiceTimer=window.setInterval(()=>{if(voiceSession)voiceTime.textContent=mediaTime((Date.now()-voiceSession.startedAt)/1000)},250)
    }catch(error:any){
    ...
    }finally{voiceBusy=false;voiceButton.disabled=false}
  }

  const cancelVoiceMessage=async()=>{
    if(voiceBusy)return
    const session=voiceSession;voiceSession=null;stopVoiceClock();setVoiceUi(false);if(session)await session.cancel()
  }

  const sendVoiceMessage=async()=>{
    if(voiceBusy||!voiceSession)return
    voiceBusy=true;voiceCancel.disabled=true;voiceSend.disabled=true
    const session=voiceSession;voiceSession=null;stopVoiceClock()
    try{
      const recording=await session.stop();setVoiceUi(false)
      if(recording.durationMs<450||recording.file.size<400){notify('Mensaje de voz muy corto','Graba un poco más antes de enviarlo.');return}
      if(recording.file.size>CHAT_AUDIO_MAX_BYTES){notify('Mensaje de voz demasiado largo',`El mensaje de voz supera ${mediaLimitMb(CHAT_AUDIO_MAX_BYTES)} MB.`);return}
      const job:PendingMedia={kind:'media',mediaKind:'audio',id:crypto.randomUUID(),file:recording.file,objectUrl:URL.createObjectURL(recording.file),replyToId:features?.getReplyToId()||null,busy:false}
      features?.clearReply();renderPendingMedia(job);void outbox.add(job)
    }catch(error){console.error('Voice recording stop failed',error);setVoiceUi(false);notify('No se pudo enviar','El mensaje de voz no pudo prepararse. Inténtalo otra vez.')}
    finally{voiceBusy=false;voiceCancel.disabled=false;voiceSend.disabled=false}
  }

  const onPendingClick=(event:Event)=>{
    const target=event.target as HTMLElement
    const mediaButton=target.closest<HTMLElement>('[data-retry-media]')
    if(mediaButton){const id=mediaButton.closest<HTMLElement>('[data-pending-id]')?.dataset.pendingId;if(id)outbox.retry(id);return}
    const textButton=target.closest<HTMLElement>('[data-retry-text]')
    if(textButton){const id=textButton.closest<HTMLElement>('[data-pending-text-id]')?.dataset.pendingTextId;if(id)outbox.retry(id)}
  }
  ...
  const synchronizeLatest=async()=>{
    ...
    try{
      ...
      for(const message of canonical){
        ...
        if(!current){await appendMessage(message,false);continue}
        const changed=current.body!==message.body||current.edited_at!==message.edited_at||current.deleted_at!==message.deleted_at||current.attachment_path!==message.attachment_path||current.attachment_type!==message.attachment_type||current.attachment_name!==message.attachment_name||current.attachment_size!==message.attachment_size
        const element=elementFor(message.id)
        const mediaNeedsRefresh=!!message.attachment_path&&!!mediaUrl(message.attachment_path)&&!element?.querySelector('.chat-attachment')
        if(changed||mediaNeedsRefresh){
          byId.set(message.id,message)
          const index=all.findIndex(item=>item.id===message.id);if(index>=0)all[index]=message
  ...
  const subscribeCore=()=>{
    coreChannel=supabase.channel(`familia-noa-chat-core-${memberId}`)
      .on('postgres_changes',{event:'INSERT',schema:'public',table:'messages'},payload=>{
        ...
        void(async()=>{if(row.attachment_path){try{await signMedia(row.attachment_path)}catch(error){console.error('Chat media authorization failed',error)}}await appendMessage(nameRow(row),false)})()
      })
      .on('postgres_changes',{event:'UPDATE',schema:'public',table:'messages'},payload=>{
        const row=payload.new as RawChatRow
        if(!row?.id||!byId.has(row.id)||closed)return
        const current=byId.get(row.id)!,next={...current,...row,sender:current.sender}
        byId.set(row.id,next);const index=all.findIndex(message=>message.id===row.id);if(index>=0)all[index]=next;replaceMessageElement(row.id)
      })
      .subscribe(status=>{if(status==='SUBSCRIBED')void synchronizeLatest()})
  }
  ...
  list.addEventListener('click',onMediaClick)
  ...
  activeCleanup=()=>{
    closed=true;stopVoiceClock()
    if(voiceSession){const session=voiceSession;voiceSession=null;void session.cancel()}
    ...
    features?.cleanup();features=null;void coreChannel?.unsubscribe();coreChannel=null
    ...
    list.removeEventListener('click',onMediaClick)
  ...
  await outbox.restore(job=>{
    if(job.kind==='media'){job.objectUrl=URL.createObjectURL(job.file);renderPendingMedia(job)}else renderPendingText(job)
  })
  if(stickToLatest)scheduleLatest()
}
